...
程序先验证 SMB URL 和挂载路径，再调用 `NetFSMountURLSync`，传入指定挂载点、空凭据和 NetAuth 无交互选项。对尚不存在的标准 `/Volumes/<共享名>` 路径，挂载点参数留空并由 NetFS 创建目录。返回成功后，程序还会检查内核挂载表中的实际路径和 SMB 来源；缺少可用钥匙串凭据时操作失败并记录错误。
...
安装器会部署编译后的命令行程序和 Swift 源码。首次安装时，它从工作区初始化运行配置。重装时，它会比较工作区与运行配置，比较时忽略配置版本和守护进程更新检查状态；两份配置相同时保留运行配置。两份有效配置不同时，交互安装会询问来源，非交互安装默认保留运行配置；`--config-source workspace` 和 `--config-source runtime` 可明确指定来源。运行配置无效或版本较新时不会静默覆盖；只有明确选择工作区配置才会执行修复。LaunchAgent 通过 `/usr/bin/swift` 启动运行目录中的源码，使网络探测能读取当前登录会话的接口作用域邻居项；编译程序供交互式命令使用。
...
* **配置管理路径**：LaunchAgent 已安装且运行配置存在时，`--config` 直接读取和编辑 `~/Library/Application Support/AutoMount/auto_mount.plist`；没有守护配置时才使用工作区配置。工作区和运行配置分别迁移。
