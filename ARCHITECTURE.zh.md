...
* **空目标截断设计**：当策略路由配置中某个高优先级策略（如 `local_lan`）的 `targets` 声明为空数组 `[]` 时，该策略即作为网络排他门牌运作。
...
在传统的配置驱动工具中，往往存在“软件版本号（如 `2.4.2`）”与“配置文件格式版本号（如 `2.0` / `2.1`）”并存的双轨制。随着业务特性的持续迭代，双轨体系会迅速带来高昂的心智与工程负债：
