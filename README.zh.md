...
- **自升级与自动更新信道 (Auto-Update Channel)**：支持安全自升级机制（`--update`），通过语义化版本（SemVer）比对 GitHub 官方发布。支持三种更新策略：`off`（默认完全关闭，零外部网络请求）、`notify`（发现新版本时发送 macOS 系统横幅提醒）、`auto`（全自动静默下载并热升级）。内置 24 小时冷却时间窗口与本地 `swiftc -parse` 编译期抽象语法树安全门，预检不通过自动终止升级，确保守护进程零崩溃风险。
...
软件版本: v2.1.0 | 自动更新信道: off (关闭自动检查，纯手动)
...
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| `version` | String | 配置文件规范版本，当前为 `2.1`。 |
